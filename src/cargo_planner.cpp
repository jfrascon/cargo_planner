...
namespace cargo_planner
{
  ...
  CargoPlanner::CargoPlanner(std::unique_ptr<PlacementHeuristic> heuristic):
    CargoPlanner(std::move(heuristic), Config{})
  {}
    ...
    cfg_(cfg)
  {
    ...
    if(!std::isfinite(cfg_.pallet_margin) || cfg_.pallet_margin < 0.0)
    {
      throw std::invalid_argument("CargoPlanner: pallet_margin must be finite and non-negative");
    }
  }
  ...
  void CargoPlanner::setFreeSpace(cv::Mat free_space, double resolution, double container_height)
  {
    ...
    if(free_space.type() != CV_8UC1)
    {
      throw std::invalid_argument("CargoPlanner::setFreeSpace: free_space must be CV_8UC1");
    }
    cv::Mat invalid_values = (free_space != 0) & (free_space != 255);
    if(cv::countNonZero(invalid_values) != 0)
    {
      throw std::invalid_argument("CargoPlanner::setFreeSpace: cells must be 0 or 255");
    }
    if(!std::isfinite(resolution) || resolution <= 0.0)
    {
      throw std::invalid_argument("CargoPlanner::setFreeSpace: resolution must be finite and positive");
    }
    if(!std::isfinite(container_height) || container_height <= 0.0)
    {
      throw std::invalid_argument("CargoPlanner::setFreeSpace: container_height must be finite and positive");
    }

    free_space_ = free_space.clone();
  ...
  {
    std::unordered_set<std::string> cargo_ids;
    for(const auto& cargo_unit: cargo_units)
    {
      if(cargo_unit.id.find_first_not_of(" \t\n\r") == std::string::npos)
      {
        throw std::invalid_argument("CargoPlanner::setCargoUnits: cargo ID must not be blank");
      }
      if(!cargo_ids.insert(cargo_unit.id).second)
      {
        throw std::invalid_argument("CargoPlanner::setCargoUnits: duplicate cargo ID '" + cargo_unit.id + "'");
      }
      if(!std::isfinite(cargo_unit.lx) || !std::isfinite(cargo_unit.ly) || !std::isfinite(cargo_unit.lz) ||
         cargo_unit.lx <= 0.0 || cargo_unit.ly <= 0.0 || cargo_unit.lz <= 0.0)
      {
        throw std::invalid_argument("CargoPlanner::setCargoUnits: cargo dimensions must be finite and positive");
      }
    }
  ...
  CargoPlanner::Result CargoPlanner::plan() const
  {
    ...
    {
      if(cargo_unit.lz > container_height_)
      {
    ...
    result.utilization_pct = (result.total_area_m2 > 0.0) ? (result.placed_area_m2 / result.total_area_m2) * 100.0 :
                                                            0.0;
  ...
  int CargoPlanner::toCells(double metres, double resolution)
  {
    ...
    if(!std::isfinite(metres) || !std::isfinite(resolution) || metres < 0.0 || resolution <= 0.0)
    {
      throw std::invalid_argument("CargoPlanner::toCells: values must be finite and valid");
    }

    const double cells = std::ceil(metres / resolution);
    if(cells > static_cast<double>(std::numeric_limits<int>::max()))
    {
      throw std::overflow_error("CargoPlanner::toCells: cell count exceeds int range");
    }
