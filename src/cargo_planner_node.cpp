...
namespace cargo_planner
{
    ...
    planner_(std::make_unique<GreedyPlacer>())
  {
    ...
    {
      throw std::invalid_argument("Parameter 'occupied_threshold' must be in [0, 100].");
    }
    if(!std::isfinite(pallet_margin_) || pallet_margin_ < 0.0)
    {
      throw std::invalid_argument("Parameter 'pallet_margin' must be finite and non-negative.");
    }

    // Build the planner with the validated parameters that belong to the core algorithm.
    ...
    std::shared_ptr<cargo_planner_msgs::srv::ContainerOccupancyGridRegistration::Response> res)
  {
    ...
    if(!std::isfinite(req->container_height) || req->container_height <= 0.0)
    {
      res->success = false;
      res->message = "Received invalid container_height. It must be positive and finite.";
      RCLCPP_WARN(get_logger(), "%s", res->message.c_str());
      return;
    }
    ...
    std::shared_ptr<cargo_planner_msgs::srv::CargoListRegistration::Response> res)
  {
    ...
    {
      res->success = false;
      res->message = std::string("Invalid cargo list: ") + e.what();
      RCLCPP_WARN(get_logger(), "%s", res->message.c_str());
    }
  }
  ...
  {
    // Keep the node alive until the detached planning thread finishes.
    auto self = std::static_pointer_cast<CargoPlacerNode>(shared_from_this());
    std::thread([self, handle]() {
      self->executePlan(handle);
    }).detach();
  }
  ...
  void CargoPlacerNode::executePlan(std::shared_ptr<GoalHandlePlanCargo> handle)
  {
    ...
    double origin_y = 0.0;
    std::string frame_id;
    nav_msgs::msg::OccupancyGrid::SharedPtr planning_grid;
    ...
    {
      result->success = false;
      result->message = "Cancelled after computation finished.";
      handle->canceled(result);
      return;
    }
    ...
    if(planning_grid)
    {
      auto updated = matToOccupancyGrid(plan_result.final_work_map, *planning_grid);
      updated.header.stamp = now;
      pub_free_map_->publish(updated);
    }

    feedback->status = "completed";
    feedback->cargo_placed = static_cast<uint32_t>(plan_result.placements.size());
    handle->publish_feedback(feedback);
