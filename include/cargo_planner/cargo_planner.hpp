 ...
 * @brief Core 2D cargo placement engine (ROS-free).
 *
 ...
 *
 * This class has no ROS dependencies.  It can be instantiated and tested
 * standalone.  The ROS node (CargoPlacerNode) wraps it and handles service
 * I/O and topic publishing.
 */
...
class CargoPlanner
{
  ...
  struct Config
  {
    /// Occupancy threshold: cells with value >= this are treated as occupied [0..100].
   ...
   * @param container_height Container interior height [m] (used for z-centre of output poses).
   */
   ...
   * @throws std::runtime_error if neither setFreeSpace() nor setCargoUnits() was called.
   */
  Result plan() const;
  ...
  cv::Mat free_space_;  ///< Binary free-space image (set by setFreeSpace).
