...
TEST(GridMapUtils, RejectsThresholdOutsideOccupancyRange)
{
  auto grid = makeGrid(1, 1, {0});
  EXPECT_THROW(occupancyGridToMat(grid, -1), std::invalid_argument);
  EXPECT_THROW(occupancyGridToMat(grid, 101), std::invalid_argument);
}
